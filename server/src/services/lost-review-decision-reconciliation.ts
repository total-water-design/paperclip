...
// Reconciliation accepts only an affirmative, canonical decision statement.
// A loose word search would accept text such as "DO NOT APPROVE" and turn an
// explicit rejection into an authoritative approval.
const CANONICAL_APPROVAL = /^\s*approve\b/i;
  ...
  return {
    reconcileApproved: async (input: LostReviewDecisionReconciliationInput) => db.transaction(async (tx) => {
      ...
      if (!evidence || evidence.deletedAt || evidence.authorType !== "user" ||
          ...
          !evidence.body.includes(input.candidate) || !CANONICAL_APPROVAL.test(evidence.body)) {
