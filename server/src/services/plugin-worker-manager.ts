...
export function createPluginWorkerHandle(
  ...
  interface HeldDuplexExitEvent {
    workerSessionId: string;
    exitCode: number | null;
    // Preserve the reason-less transport-close discriminator while an exit
    // arrives before the open reply has bound its worker-session pair.
  ...
  function bufferPreBindDuplexFrame(
    ...
    if (notification.method === DUPLEX_CHANNEL_EXIT_NOTIFICATION) {
      ...
      route.preBindExit = {
        workerSessionId,
        exitCode,
        transportClosed: params.transportClosed === true ? true : undefined,
      };
      return;
    }
