...
describeEmbeddedPostgres("stale issue execution lock routes", () => {
  ...
  it("self-heals a stale checkoutRunId via clearCheckoutRunIfTerminal on checkout (Fix B path)", async () => {
    ...
    const otherAgentCurrentRunId = randomUUID();
    await db.insert(agents).values({
      id: otherAgentId,
      companyId,
      name: "OtherAgent",
    ...
    const res = await request(createApp(agentActor(companyId, otherAgentId, otherAgentCurrentRunId)))
      .post(`/api/issues/${issueId}/checkout`)
      .send({
        agentId: otherAgentId,
        expectedStatuses: ["todo", "backlog", "blocked", "in_review"],
    ...
    expect(row).toEqual({
      status: "in_progress",
      assigneeAgentId: otherAgentId,
      checkoutRunId: otherAgentCurrentRunId,
      executionRunId: otherAgentCurrentRunId,
    });
  });
});
