...
export function issueRoutes(
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      // Retry the durable, idempotency-keyed dependency wake even when the
      // decision already exists. A prior request can commit the transaction
      // and then lose the enqueue response. Replay must repair that gap.
      ...
      for (const dependent of dependents) {
        try {
          ...
          logger.warn({ err, issueId: issue.id, dependentIssueId: dependent.id },
            "failed to enqueue lost-review reconciliation dependency wake");
          throw err;
        }
      }
