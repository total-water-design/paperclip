...
describe("root systemd preflight", () => {
  ...
  it("requires the unit to execute the fail-closed preflight and parameterized executable", () => { const unit = fs.readFileSync(path.resolve(import.meta.dirname, "../../../deploy/systemd/paperclip.service"), "utf8"); expect(unit).toContain("ExecStartPre=/usr/lib/paperclip/paperclip-preflight"); expect(unit).toContain("Environment=PAPERCLIP_ENV_FILE=/etc/paperclip/paperclip.env"); expect(unit).toContain('"$PAPERCLIP_EXECUTABLE" run --instance "$PAPERCLIP_INSTANCE_ID"'); expect(unit).not.toContain("/usr/local/bin/paperclipai"); });
});
