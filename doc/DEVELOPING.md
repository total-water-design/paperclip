...
- `pnpm paperclipai worktree ensure-seeded` performs the deferred seed **exactly once**. It is lock-guarded and idempotent: only a complete `verified` manifest short-circuits it, so it is safe to call repeatedly and from concurrent processes. On Linux, a pending `full` seed is re-executed in a Paperclip-managed transient user service before it acquires the seed lock; this lets a long logical snapshot survive its foreground heartbeat/client process. The service must leave a verified manifest or the caller records a terminal failed diagnostic. Managed workspaces derive the source from the control-plane-provided base project workspace when it carries its own `.paperclip/config.json`, and otherwise from the control plane's own registered instance config; either way the workspace's manifest never selects it. Manual worktrees must pass `--from-config`.
