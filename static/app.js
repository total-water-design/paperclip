...
const multistageEnergy={...energy,
  pump_curve_basis:{label:'HPP pump selection mode',type:'select',options:[['shared_auto','Auto · VCMP / HHECP / PD'],['vcmp','VCMP only'],['hhecp','HHECP only'],['pd','Positive displacement only'],['manual','Manual efficiency / BEP']],info:'Auto evaluates the existing shared pump engine across VCMP, HHECP, and positive-displacement technologies. Technology-specific modes route only to that catalog. If the duty is too small or no option is feasible, Total RO Design explicitly reports the fallback and uses the entered manual efficiency.'},
...
const pxEnergy={
  pump_curve_basis:{...multistageEnergy.pump_curve_basis},
...
function multistagePumpCurveChart(r){
  ...
  const fu=r.flow_unit,pu=r.pressure_unit,isDb=['vcmp_database','shared_pump_database'].includes(r.pump_curve_source),displayPts=!!r.pump_curve_points_are_display_units;
  ...
  if(isDb){
    ...
    return `<section class="report-section vcmp-report-section"><div class="chart-title-row"><h3>HPP SHARED PUMP CURVE / DUTY POINT</h3><span>${escapeHtml(r.pump_selected_technology||'Catalog')} · ${escapeHtml(zone)}</span></div><div class="cp-legend suite-cp-legend"><span class="legend-mono">Selected pump curve</span><span class="legend-di">RO system curve</span></div><div class="flux-chart-wrap vcmp-chart-wrap"><svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Selected pump curve and RO system curve">${grid.join('')}<line x1="${L}" y1="${H-B}" x2="${W-R}" y2="${H-B}" class="flux-axis"/><line x1="${L}" y1="${T}" x2="${L}" y2="${H-B}" class="flux-axis"/><path d="${pumpPath}" class="cp-line cp-line-mono"/><path d="${sysPath}" class="cp-line cp-line-di"/>${op}<text x="${(L+W-R)/2}" y="${H-7}" text-anchor="middle" class="flux-axis-title">Flow (${escapeHtml(fu)})</text><text x="15" y="${(T+H-B)/2}" transform="rotate(-90 15 ${(T+H-B)/2})" text-anchor="middle" class="flux-axis-title">Differential pressure (${escapeHtml(pu)})</text></svg></div><div class="duty-actions vcmp-duty-actions"><div><span>Selected pump</span><strong>${escapeHtml(r.pump_selected_model||r.pump_selected_family||'Catalog pump')}</strong></div><div><span>Duty / installed units</span><strong>${fmt(r.pump_duty_units||1,0)} / ${fmt(r.pump_installed_units||1,0)}</strong></div><div><span>Pump efficiency</span><strong>${fmt(100*r.pump_operating_efficiency,1)}%</strong></div><div><span>Wire power</span><strong>${fmt(r.pump_operating_wire_kw,2)} kW</strong></div><div><span>Shaft torque</span><strong>${torqueText}</strong></div><div><span>Required / generated ΔP</span><strong>${fmt(r.pump_duty_dp,2)} / ${fmt(r.pump_generated_dp,2)} ${escapeHtml(pu)}</strong></div></div>${opts?`<div class="table-wrap vcmp-options-table"><table class="fedco-table"><tr><th>Rank</th><th>Pump option</th><th>Speed</th><th>Pump η</th><th>Wire power</th><th>Operating note</th></tr>${opts}</table></div>`:''}<p class="micro-note">Selection is routed through the shared VCMP/HHECP/PD pump authority using the requested technology mode. Final selection must verify NPSH, minimum continuous flow, motor rating, materials, pressure rating and mechanical seal.</p></section>`;
  }
...
function multistagePumpReport(r){
  const hppDb=['vcmp_database','shared_pump_database'].includes(r.pump_curve_source);
  const hppSel=hppDb?`${escapeHtml(r.pump_selected_technology||'VCMP')} · ${escapeHtml(r.pump_selected_model||r.pump_selected_family||'Catalog pump')} · η ${pct(r.pump_operating_efficiency)}% · ${fmt(r.pump_duty_units||1,0)} duty unit(s)`:'Fallback entered efficiency / manual screening';
  ...
  const dbNote=hppDb?`HPP selected through the shared pump authority in ${escapeHtml(r.pump_selection_mode||'catalog')} mode; technology ${escapeHtml(r.pump_selected_technology||'not reported')}.`:(r.pump_database_fallback?`No feasible catalog option covered the HPP duty; entered manual efficiency remains active. ${escapeHtml(r.pump_database_fallback_reason||'')}`:'Manual pump screening / entered efficiency is active.');
